...
create_instruction(const ParsedLine &pl,
  ...
  if (op_name == "slice") {
    ...
    int64 scale = 0;
    {
      std::stringstream ss(attr_str);
      std::string tok;
      if (!std::getline(ss, tok, ',')) return panic("missing rows attribute");
      try { rows = std::stoll(tok); } catch (...) { return panic("failed to parse rows attribute"); }
      if (!std::getline(ss, tok, ',')) return panic("missing cols attribute");
      try { cols = std::stoll(tok); } catch (...) { return panic("failed to parse cols attribute"); }
      if (!std::getline(ss, tok, ',')) return panic("missing scale attribute");
      try { scale = std::stoll(tok); } catch (...) { return panic("failed to parse scale attribute"); }
    }

    return new MvinSpad(lhs_tensor, rhs_tensors, rows, cols, scale);
  }
  else if (op_name == "mvout_spad") {
    int64 rows = 0;
    int64 cols = 0;
    {
      std::stringstream ss(attr_str);
    ...
    return new MvoutSpad(lhs_tensor, rhs_tensors, rows, cols);
  }
  else if (op_name == "mvout_spad_relu") {
    int64 rows = 0;
    int64 cols = 0;
    {
      std::stringstream ss(attr_str);
    ...
    return new MvoutSpadRelu(lhs_tensor, rhs_tensors, rows, cols);
  }
  else if (op_name == "mvin_acc") {
    int64 rows = 0;
    int64 cols = 0;
    int64 scale = 0;
    {
    ...
    return new MvinAcc(lhs_tensor, rhs_tensors, rows, cols, scale);
  }
  else if (op_name == "mvout_acc") {
    int64 rows = 0;
    int64 cols = 0;
    {
      std::stringstream ss(attr_str);
    ...
    return new MvoutAcc(lhs_tensor, rhs_tensors, rows, cols);
  }
  else if (op_name == "mvout_acc_relu") {
    int64 rows = 0;
    int64 cols = 0;
    {
      std::stringstream ss(attr_str);
    ...
    return new MvoutAccRelu(lhs_tensor, rhs_tensors, rows, cols);
  }
  else if (op_name == "mvin_acc_low") {
    int64 rows = 0;
    int64 cols = 0;
    int64 scale = 0;
    {
    ...
    return new MvinAccLow(lhs_tensor, rhs_tensors, rows, cols, scale);
  }
  else if (op_name == "mvin_acc_low_add") {
    int64 rows = 0;
    int64 cols = 0;
    int64 scale = 0;
    {
    ...
    return new MvinAccLowAdd(lhs_tensor, rhs_tensors, rows, cols, scale);
  }
  else if (op_name == "mvout_acc_low") {
    int64 rows = 0;
    int64 cols = 0;
    {
      std::stringstream ss(attr_str);
    ...
    return new MvoutAccLow(lhs_tensor, rhs_tensors, rows, cols);
  }
  else if (op_name == "mvout_acc_low_relu") {
    int64 rows = 0;
    int64 cols = 0;
    {
      std::stringstream ss(attr_str);
    ...
    return new MvoutAccLowRelu(lhs_tensor, rhs_tensors, rows, cols);
  }
  else if (op_name == "matmul8") {
    int64 DIM_I = 0;
    int64 DIM_J = 0;
    int64 DIM_K = 0;
    {
      std::stringstream ss(attr_str);
      std::string tok;
      if (!std::getline(ss, tok, ',')) return panic("missing DIM_I attribute");
      try { DIM_I = std::stoll(tok); } catch (...) { return panic("failed to parse DIM_I attribute"); }
      if (!std::getline(ss, tok, ',')) return panic("missing DIM_J attribute");
      try { DIM_J = std::stoll(tok); } catch (...) { return panic("failed to parse DIM_J attribute"); }
      if (!std::getline(ss, tok, ',')) return panic("missing DIM_K attribute");
      try { DIM_K = std::stoll(tok); } catch (...) { return panic("failed to parse DIM_K attribute"); }
    }

    return new Matmul8(lhs_tensor, rhs_tensors, DIM_I, DIM_J, DIM_K);
  }
  else if (op_name == "matmul32") {
    int64 DIM_I = 0;
    int64 DIM_J = 0;
    int64 DIM_K = 0;
    {
    ...
    return new Matmul32(lhs_tensor, rhs_tensors, DIM_I, DIM_J, DIM_K);
  }
  else if (op_name == "mac8") {
    int64 DIM_I = 0;
    int64 DIM_J = 0;
    int64 DIM_K = 0;
    {
    ...
    return new Mac8(lhs_tensor, rhs_tensors, DIM_I, DIM_J, DIM_K);
  }
  else if (op_name == "mac32") {
    int64 DIM_I = 0;
    int64 DIM_J = 0;
    int64 DIM_K = 0;
    {
    ...
    return new Mac32(lhs_tensor, rhs_tensors, DIM_I, DIM_J, DIM_K);
  }
