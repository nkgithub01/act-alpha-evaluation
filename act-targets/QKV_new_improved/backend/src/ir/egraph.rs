...
impl Language for TensorOp {
    ...
    fn children(&self) -> &[Id] {
        match self {
            ...
            TensorOp::AlphaD3(id) => std::slice::from_ref(id),
    ...
    fn children_mut(&mut self) -> &mut [Id] {
        match self {
            ...
            TensorOp::AlphaD3(id) => std::slice::from_mut(id),
...
impl FromOp for TensorOp {
    ...
    fn from_op(op: &str, children: Vec<Id>) -> Result<Self, Self::Error> {
        match op {
            ...
            op if op == "alpha-d3" && children.len() == 1 => {
                Ok(TensorOp::AlphaD3(children[0]))
            }
...
impl std::fmt::Display for TensorOp {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            TensorOp::AlphaHBM(_) => write!(f, "alpha-hbm"),
            TensorOp::AlphaD1(_) => write!(f, "alpha-d1"),
            TensorOp::AlphaD3(_) => write!(f, "alpha-d3"),
            TensorOp::AlphaD2(_) => write!(f, "alpha-d2"),
...
impl Analysis<TensorOp> for TensorInfo {
    ...
    fn make(egraph: &mut EGraph<TensorOp, Self>, enode: &TensorOp) -> Self::Data {
        match enode {
            ...
            TensorOp::AlphaHBM(child)
            | TensorOp::AlphaD1(child)
            | TensorOp::AlphaD3(child)
            | TensorOp::AlphaD2(child) => egraph[*child].data.clone(),
